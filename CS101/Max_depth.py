...
def crawl_web(seed, max_depth):
    ...
    while tocrawl:
        page_with_depth = tocrawl.pop(0)
        if page_with_depth[0] not in crawled and page_with_depth[1] <= max_depth:
            depth = page_with_depth[1] + 1
            all_links = get_all_links(get_page(page_with_depth[0]))
            table = []
            for e in all_links:
                table.append([e, depth])
            union(tocrawl, table)
            crawled.append(page_with_depth[0])
    return crawled
